...
series_labels = ['greedy','mdp','pomcp','despot','oracle']
series_colors = ['green','purple','orange','pink','blue']
ylabel = 'Avg. Reward'

alg_means = [list(),list(),list(),list(),list()]
alg_stds = [list(),list(),list(),list(),list()]
categories = list()
...
for size in SIZES:
  for of in OBSFRACS:
    ...
    with open(fname,'r') as f_greedy:
      ...
      alg_means[4].append(info_list[2])
      alg_stds[4].append(0.0)
    ...
    fname = 'ARDESPOT-'+str(size)+'-'+str(of)+'.txt'
    with open(fname,'r') as f_despot:
      info = f_despot.readline()
      info_list = [float(v) for v in info.split(',')]
      alg_means[3].append(info_list[0])
      alg_stds[3].append(info_list[1])
...
output(fig,'performance_nfz20_withdespot.pdf',size=(10.2,4.5),latex=False,fontsize=16,legend_fontsize=14)